<x-guest-layout>
        <div class="relative flex items-top justify-center min-h-screen sm:items-center py-4 sm:pt-0">
            ...
            <div class="max-w-6xl  mx-auto sm:px-6 lg:px-8">
                ...
                @if (Route::has('login'))
                        @auth
                            <a href="{{ url('/dashboard') }}" class="bg-brand_blue text-sm text-brand_yellow hover:text-brand_orange hover:scale-110 px-8 py-4 rounded-lg hover:bg-brand_green transition-all duration-500 shadow hover:shadow-xl uppercase font-bold shadow-brand_yellow">
                            {{__('Dashboard')}}
                            </a>
                        @else
